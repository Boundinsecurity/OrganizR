...
const Navbar = ({ user, onLogout }) => {
  const safeUser = user ?? {};           // <- ensure object, handles null/undefined
      ...
      <div className='flex items-center justify-between px-4 py-3 md:px-6 max-w-7xl mx-auto'>
        ...
        <div className='flex items-center gap-4'>
          ...
          <div ref={menuref} className='relative '>
            <button onClick={handleMenuToggle} className='flex items-center gap-2 px-3 py-2 rounded-full cursor-pointer hover:bg-purple-50
                  transition-colors duration-300 border border-transparent hover:border-purple-200'>
              <div className='relative'>
                { safeUser?.avatar ? (
                  <img src={safeUser.avatar || '/default-avatar.png'} alt="avatar"  className='w-9 h-9 rounded-full shadow-sm '/>
                ) : (
                  <div className='w-8 h-8 flex items-center justify-center rounded-full bg-gradient-to-br from-fuchsia-500 to-purple-600
                        text-white font-semibold shadow-md'>
                    { safeUser?.name?.[0]?.toUpperCase() || 'U' }
                  </div>
                )}
              ...
              <div className='text-left hidden md:block'>
                <p className='text-sm font-medium text-gray-800'>{safeUser?.name}</p>
              ...
              <ul className='absolute top-14 right-0 w-56 bg-white rounded-2xl shadow-xl border border-purple-100 z-50 overflow-hidden
                ...
                <li className='p-2'>
                  <button onClick={() => {
                    ...
                    className='w-full px-4 py-2.5 text-left hover:bg-purple-50 text-sm text-gray-700 transition-colors 
